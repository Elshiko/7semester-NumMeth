      ...
      with open(filename, 'r') as file_with_data:
        ...
        for row in range(0, len(data)):
          ...
          for col in range(0, len(data[row])):
            matr_list[row].append(int(data[row][col]))
          vect_list.append(int(data[row][-1]))
    else:
      data = input().split(separator)
      matr_list.append([])
      dim = len(data)
      for col in range(0, dim - 1):
        matr_list[0].append(int(data[col]))
      vect_list.append(int(data[-1]))
      for row in range(1, dim):
        data = input().split(separator)
        matr_list.append([])
        for col in range(0, dim - 1):
          matr_list[row].append(int(data[col]))
        vect_list.append(int(data[dim - 1]))
      ...
      with open(filename, 'r') as file_with_data:
        ...
        for row in range(0, dim):
          ...
          for col in range(0, dim):
            matr_list.append(int(data[row][col]))
        for num in range(0, dim):
          vect_list.append(int(data[-1][num]))
    else:
      dim = int(input())
      for row in range(0, dim):
        matr_list.append([])
        line = input().split(' ')
        for col in range(0, dim):
          matr_list[row].append(int(line[col]))
      data = input().split(' ')
      for num in range(0, dim):
        vect_list.append(int(data[num]))
